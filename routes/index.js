...
function processV1Request(prequest, presponse) {
    ...
    function searchIntent(app, parameters) {
        ...
        let reqURL = "https://api.ivi.ru/mobileapi/search/v5/?from=0&to=20&age=16&app_version=870&query="
    ...
    function welcomeIntent(app) {
        ...
        app.ask(
            app.buildRichResponse()
                ...
                .addSuggestions(['Новинки', 'Порекомендуй что-нибудь', 'Чем я могу помочь?', 'Лучшие сериалы', 'Фильмы', 'Для детей'])
    ...
    function similarIntent(inputContexts, parameters, app) {
        ...
        let reqURL = "https://api.ivi.ru/mobileapi/search/v5/?from=0&to=20&age=16&app_version=870&fields=id,title&query="
            + encodeURIComponent(contextSearchResult);
        let u = id === -1 ? reqURL : byIdUrl;
        console.log('url=' + u);
        let isById = id !== -1;
